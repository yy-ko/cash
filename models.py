...
#  class MultiLayerModel(nn.Module):
    #  def __init__(self, model: nn.Module, inputs, n_layers: int = 2):
    ...
    #  def forward(self, inputs, n_layers):
        ...
        #  classifier_layers = [self.encoder.h_dim, 1]
        #  Layers = []
        #  for i in range(len(classifier_layers)-1):
            #  Layers.append(nn.Linear(classifier_layers[i], classifier_layers[i+1]))
